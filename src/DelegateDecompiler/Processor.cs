...
namespace DelegateDecompiler
{
    internal class Processor
    {
        class ProcessorState
        {
            ...
            {
                // Ugly hack to handle NewArrayBounds along NewArrayInit
                if (Stack.Peek().Expression?.NodeType == ExpressionType.NewArrayInit)
                {
                    return Stack.Pop();
                }
                var outputExpression = Stack.Reverse().Where(it => it.Expression != null && (it.Expression.NodeType != ExpressionType.Default || it.Type != typeof(void))).Select(address => address.Expression).ToList();
                switch (outputExpression.Count())
                {
                    case 0: return Expression.Empty();
                    case 1: Stack.Pop(); return outputExpression.First();
                    default:
                        return Expression.Block(outputExpression);
                }
            }
        }
        ...
        Expression Process()
        {
            ...
            while (states.Count > 0)
            {
                ...
                if (state.Instruction != null && state.Instruction != state.Last)
                {
                    ...
                    else if (state.Instruction.OpCode == OpCodes.Leave || state.Instruction.OpCode == OpCodes.Leave_S)
                    {
                        var blockInstructions = states.Pop().Stack.Reverse().Select(it => it.Expression).ToArray();
                        state.Stack.Clear();
                        state.Stack.Push(blockInstructions.Count() > 1 ? Expression.Block(blockInstructions) : blockInstructions.First());
                    }
        ...
        internal static Expression AdjustType(Expression expression, Type type)
        {
            ...
            if (type != typeof(void) && expression.Type != typeof(void) && type.IsValueType != expression.Type.IsValueType)
            {
                return Expression.Convert(expression, type);
            }
